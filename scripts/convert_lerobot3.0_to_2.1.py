...
def _sanitize_table_for_legacy_parquet(table):
    """Drop schema metadata to avoid datasets feature-metadata incompatibilities.

    openpi's current dependency stack reads per-episode parquet files with
    `datasets.load_dataset("parquet", ...)`. Newer parquet metadata can embed
    feature descriptors that are not recognized by the pinned datasets version
    (e.g. `_type: "List"`), which raises at load time. Removing schema metadata
    keeps raw Arrow columns intact while letting datasets infer compatible
    features from the physical schema.
    """
...
def convert_data(
    ...
    for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc="convert data files"):
        ...
        for record in records:
            ...
            episode_table = _sanitize_table_for_legacy_parquet(episode_table)
